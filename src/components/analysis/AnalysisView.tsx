...
export const AnalysisView: React.FC<AnalysisViewProps> = ({
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-24 fade-in">
      ...
      {/* Quick Setup Card if API Key is missing */}
      {!hasApiKey && (
        <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-3xl p-5 sm:p-6 shadow-soft space-y-4 print:hidden">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className="text-sm font-bold text-amber-900">
                Configura la tua Chiave Google Gemini gratuita
              </h3>
              <p className="text-xs text-amber-700 mt-1 leading-relaxed">
                Per generare report illimitati a costo zero, inserisci la tua API Key personale ottenibile in 1 minuto da Google AI Studio. Rimane salvata solo sul tuo dispositivo.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <input
              type="password"
              placeholder="Incolla la tua chiave API (es. AIzaSy...)"
              value={apiKeyInput}
              onChange={(e) => setApiKeyInput(e.target.value)}
              className="flex-1 px-4 py-2.5 rounded-2xl bg-white border border-amber-200 text-xs font-mono focus:outline-none focus:ring-2 focus:ring-amber-500"
            />
            <button
              type="button"
              disabled={isTestingKey}
              onClick={handleSaveApiKey}
              className="px-5 py-2.5 rounded-2xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold transition-all shadow-sm flex items-center justify-center gap-1.5 disabled:opacity-50"
            >
              {isTestingKey ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Check className="w-3.5 h-3.5" />}
              <span>Salva e Attiva</span>
            </button>
          </div>
          ...
          <div className="flex items-center justify-between text-[11px] text-amber-800 pt-1 border-t border-amber-200/60">
            ...
            >
              <span>Ottieni la chiave gratuita su Google AI Studio</span>
