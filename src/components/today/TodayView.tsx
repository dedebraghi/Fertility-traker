...
export const TodayView: React.FC<TodayViewProps> = ({
  ...
  const [transitionCycleNumber, setTransitionCycleNumber] = useState<number>(estToday.cycleNumber);
  ...
  // Sync selectedDay to today's cycle day on load
  useEffect(() => {
    if (activeCycle && !isCycleStale && calculatedDayToday && calculatedDayToday >= 1 && calculatedDayToday <= 50) {
      setSelectedDay(calculatedDayToday);
    } else {
      setSelectedDay(estToday.cycleDay || 1);
    }
  }, [activeCycle?.id, isCycleStale]);
  ...
  const handleMenstruationChange = (newMenst: DailyEntry['menstruation']) => {
    ...
    if (newMenst && ['Flusso', 'Abbondante', 'Spotting', 'M', 'M+', 'm'].includes(newMenst)) {
      const targetDate = calculateDateForDay(currentCycleStartDate, selectedDay) || todayISO;
      const nextNum = isCycleStale ? estToday.cycleNumber : (activeCycle ? activeCycle.cycle_number + 1 : 1);
  ...
  const currentDateStr = calculateDateForDay(currentCycleStartDate, selectedDay) || todayISO;
          ...
          <div className="text-center">
            <div className="flex items-center justify-center gap-1.5 mb-0.5">
              <span className="text-xs uppercase font-extrabold tracking-widest text-nature-200">
                Ciclo #{currentCycleNumber} • Giorno del Ciclo
              </span>
              {isToday && (
      ...
      <div className="sticky bottom-20 z-30 pt-2">
        ...
        >
          {saving ? (
            ...
            <>
              <Sparkles className="w-5 h-5" />
              <span>Salva Misurazioni Giorno {selectedDay}</span>
            </>
          )}
        </button>
      </div>

      {/* Intelligent Cycle Transition Modal (Triggered ONLY when registering menstruation) */}
      {showTransitionModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-sm fade-in">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl border border-nature-100 slide-up">
            ...
            <p className="text-xs text-stone-600 text-center mb-6 leading-relaxed">
              ...
              Vuoi far partire il <strong>Ciclo {transitionCycleNumber}</strong> con inizio in questa data?
            </p>
            ...
            <div className="space-y-2.5">
              ...
              >
                Continua nel ciclo attuale ({currentCycleNumber})
              </button>
            </div>
          </div>
        </div>
